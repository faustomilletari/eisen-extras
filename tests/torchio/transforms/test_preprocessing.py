...
class TestRescaleIntensity:
    def setup_class(self):
        ...
        self.subject = Subject(
            data=Image(
                tensor=self.data["data"], type=torchio.INTENSITY
            ),  # 4D input required
            mask={"data": torch.from_numpy(self.data["mask"][np.newaxis])},
        )

    def test_min_max(self):
        tform_ref = TorchIORescaleIntensity(out_min_max=[0.0, 1.0])

        tform_tst = RescaleIntensity(["data"], out_min_max=[0.0, 1.0])
    ...
    def test_percentiles(self):
        tform_ref = TorchIORescaleIntensity(
            out_min_max=[0.0, 1.0], percentiles=(0.05, 0.95)
        )

        tform_tst = RescaleIntensity(
            ["data"], out_min_max=[0.0, 1.0], percentiles=(0.05, 0.95)
        )
    ...
    def test_with_mask(self):
        tform_ref = TorchIORescaleIntensity(
            out_min_max=[0.0, 1.0], masking_method="mask"
        )

        tform_tst = RescaleIntensity(
            ["data"], out_min_max=[0.0, 1.0], mask_field="mask"
        )
...
class TestZNormalization:
    ...
    def test_mask(self):
        tform_ref = TorchIOZNormalization(masking_method="mask")

        tform_tst = ZNormalization(["data"], mask_field="mask")
